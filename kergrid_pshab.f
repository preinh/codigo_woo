...
C Read event Date, Longitude, Latitude, Magnitude, Lineament angle      
      READ(5,990) IYR(I),IM(I),ID(I),KDEP,QKE(I,1),QKE(I,2),QKE(I,3)
      DEEP(I) = KDEP
      TH(I) = 0.0
...
C
      ...
      IF ( (QKE(I,3).GE.3.5).AND.(QKE(I,3).LT.4.0) ) YR(I) = IPER35
      IF ( (QKE(I,3).GE.3.0).AND.(QKE(I,3).LT.3.5) ) YR(I) = IPER30
C
C
...
C Iterate over the regional catalogue events
      DO 200 I=1,NPT
      IF (QKE(I,3).LT.3.0) GOTO 200
      IF (DEEP(I).GT.600.0) GOTO 200
...
C to the grid activity rate density
      ...
      IF (DISG.GT.4000.0) GOTO 400
C
C Consider different magnitude values, ranging from 3.0 up to 6.75
      DO  500 MG = 1,8
C GMAG is the magnitude associated with the intensity distribution
      GMAG = 3.0 + (MG-1)*2.0*DELTA
C
...
674   FORMAT(//,6X,'  ACTIVITY RATES AGGREGATED OVER THE MAIN BLOCK') 
      WRITE(*,675)
675   FORMAT(6X,'3.0         3.50        4.00         4.50')
      WRITE(*,676) (TOTMAG(K),K=1,4)
      WRITE(*,677)
677   FORMAT(//,6X,'5.00       5.50        6.00        6.50')
      WRITE(*,676) (TOTMAG(K),K=5,8)
676   FORMAT(8E12.2)
      WRITE(*,678) SUM
678   FORMAT(//'RATE OF ACTIVITY OF EVENTS > 3.0:',E12.2)
C      
      STOP
      END
C
